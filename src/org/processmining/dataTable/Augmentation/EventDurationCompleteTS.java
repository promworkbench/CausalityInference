...
public class EventDurationCompleteTS {
	...
	/**
	 * This field indicates according to the tokens in the model each transition can be fired how many times
	 */
	private Map<Transition, Integer> activeTransitions = new HashMap<Transition, Integer>(); 
	
	/**
	 * This field indicates when each transition was activated.
	 */
	private Map<Transition, LinkedList<Long>> activationTimeTransitions = new HashMap<>(); 
	...
	public EventDurationCompleteTS(Petrinet model) {
		this.model = model;
		setOutgoingPlaces();
		setIncomingPlaces();
	...
	public void setActivityDuration(XTrace trace, SyncReplayResult srr) {
		...
		// add a token to each start place
		for (Place place : startPlaces) { 
			numTokenInPlaces.put(place,  1);
		}
		// which transitions are activated at the beginning
		for (Transition transition : model.getTransitions()) { 
			boolean flag = true;
			...
			if (flag) {
				...
				list.add((long) 0);
				activationTimeTransitions.put(transition, list);
			}
		}
	...
	public void updatePlaceTokens(Transition thisTransition) {
		...
		// update output places of the invisible transition
		for (Place place : outputPlacesOfTransition.get(thisTransition)) { 
		...
		// update out put places of the invisible transition
    	for (Place place : inputPlacesToTransition.get(thisTransition)) { 
    		Integer num = numTokenInPlaces.get(place);
    		if (num != null && num >= 1) {
    			num--;
    			numTokenInPlaces.remove(place);
	...
	public void updateActiveTransitions() {
		Map<Transition, Integer> oldActiveTransitions = activeTransitions;
		activeTransitions = new HashMap<Transition, Integer>();
		for (Transition transition : model.getTransitions()) {
			Integer min = 0;
			// put the number of tokens of one of the input places of the transition as the minimum 
			for (Place p : inputPlacesToTransition.get(transition)) {   
		...
		updateStartTime(oldActiveTransitions); //added
	}
	
	private void updateStartTime(Map<Transition, Integer> oldActiveTransitions) {
		LinkedList<Long> list = new LinkedList<>();
		list.add((long) 0);
	//	activationTimeTransitions.put(transition, list);
